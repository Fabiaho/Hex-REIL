...
class ConvolutionalPolicy(nn.Module):
    def __init__(self, size: int, width: int = 32, n_heads: int = 4, layers: int = 2):
        super(ConvolutionalPolicy, self).__init__()
        self.input_layer = nn.Conv2d(1, width, kernel_size=3, padding=1)
...
def make_torch_net(input_length: int, width: int, output_length: int, hidden=3):
    net = ConvolutionalPolicy(size=input_length, width=width, n_heads=width//16, layers=hidden)
    print(net)
    return net
...
class deepQ(object):
    ...
    def initialize_networks(self, width=128, hidden=3):
        self.policy_net = make_torch_net(
            input_length=self.n_observations,
            width=width,
            output_length=self.n_actions,
        ...
        for i_episode in range(num_episodes):
            ...
            if random.random() < 0.5:
                ...
                if opponents is None:
                    ...
                    p = np.array(list(opponent_weights) + [opponent_weights[0]*0.9])
                    p /= p.sum()
                    opponent = np.random.choice(selection_pool, p=p)
                if opponent is None:
...
if __name__ == "__main__":
    folder = "deepq_models/deepq_convolutional_5x5"
    init_model = None
    size = 5
    learning_rate = 1e-3
    gamma = 0.9
    ...
    for i in range(0, 50):
        ...
        for file in pth_files:
            ...
            if "deepq_hex_base" in str(file):
                idx = 0
            else:
                idx = int(str(file).split("_")[-1].split(".")[0])+1
            trained_agents.append((trained_agent, idx))
        trained_agents = sorted(trained_agents, key=lambda x: x[1])
        sum_idx = sum([idx for _, idx in trained_agents])
        agent_weights = np.zeros(len(trained_agents))
        weight = 1.0
        for i in range(len(trained_agents)):
            agent_weights[-i] = weight
            weight *= 0.9
        agent_weights = agent_weights / sum(agent_weights)
        trained_agents = [agent for agent, _ in trained_agents]
        ...
        agent.learn(
            ...
            opponent_weights=agent_weights,
            visualize=visualise,
            learning_rate=learning_rate,
            gamma=gamma,
            early_stopping_condition=early_stopping_condition,
