...
class hexPosition(object):
    ...
    def moove(self, coordinates):
        ...
        ), "These coordinates already contain a stone."
    ...
    def _get_adjacent(self, coordinates):
        ...
        return [
            pair
            for pair in [u, d, r, l, ur, dl]
            if max(pair[0], pair[1]) <= self.size - 1 and min(pair[0], pair[1]) >= 0
        ]
    ...
    def _prolong_path(self, path):
        ...
        candidates = [
            cand for cand in candidates if self.board[cand[0]][cand[1]] == player
        ]
    ...
    def machine_vs_machine(self, machine1=None, machine2=None, interactive=True):
        """
        ...
        while self.winner == 0:
            ...
            if self.winner == 1:
                if interactive:
                    self.print()
                self._evaluate_white(verbose=interactive)
            if self.winner == -1:
                if interactive:
                    self.print()
                self._evaluate_black(verbose=interactive)
