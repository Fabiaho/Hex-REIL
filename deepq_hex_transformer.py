...
class TransformerPolicy(nn.Module):
    def __init__(self, size: int, width: int = 32, n_heads: int = 4, layers: int = 2):
        ...
        pos_z = -pos_x - pos_y
        self.pos_encoding = torch.stack([pos_x, pos_y, pos_z], dim=2)
        self.input_embedding = nn.Linear(4, width)
    ...
    def forward(self, x):
        ...
        x = torch.cat([x.unsqueeze(2), pos_encoding], dim=2)
...
if __name__ == "__main__":
    folder = "deepq_models/deepq_transformers_7x7_4"
    init_model = None
    size = 7
    learning_rate = 1e-3
    gamma = 0.9
