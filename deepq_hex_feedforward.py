...
class FeedForwardPolicy(nn.Module):
    def __init__(self, size: int, width: int = 32, n_heads: int = 4, layers: int = 2):
        ...
        self.input_layer = nn.Linear(size , width)
        self.hidden_layers = nn.ModuleList(
            [nn.Linear(width, width) for _ in range(layers)]
        )
        self.output_layer = nn.Linear(width, size)
...
def make_torch_net(input_length: int, width: int, output_length: int, hidden=1):
    net = FeedForwardPolicy(size=input_length, width=width, n_heads=width//16, layers=hidden)
    print(net)
    return net
...
class deepQ(object):
    ...
    def initialize_networks(self, width=128, hidden=2):
        self.policy_net = make_torch_net(
            input_length=self.n_observations,
            width=width,
            output_length=self.n_actions,
...
if __name__ == "__main__":
    folder = "deepq_models/deepq_feedforward_7x7_3"
    init_model = None
    size = 7
    learning_rate = 1e-3
    gamma = 0.9
